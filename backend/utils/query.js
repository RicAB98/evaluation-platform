...
const query = {
  ...
  getQuerySummary(string, startDate, nextDay, last24Hours, last7Days) {
   ...
   /* let timeConditions = `date = '${startDate.getFullYear()}-${startDate.getMonth()}-${startDate.getDate()}'`;
    ...
    return `select search_string, sumRank, oneCount, totalClicks, totalLast24h, totalPrevious24h, totalLast7days, average7days, cast(((totalLast24h-totalPrevious24h)/totalPrevious24h  * 100) as decimal(10,2)) as GrowthLast24h, cast(((totalLast24h-average7days)/average7days * 100) as decimal(10,2)) as GrowthLast7d FROM 
    (select search_string, count(*) as totalLast7days, count(*)/7 as average7days from fourdays where date > ${last7Days} and date < ${nextDay} and search_string = '${string}') as 7days natural join 
    (select search_string, count(*) as totalPrevious24h from fourdays where date = ${last24Hours} and search_string = '${string}') as 24hours natural join  
    (select search_string, count(*) as totalLast24h from fourdays where date = ${startDate} and search_string <> '' and search_string = '${string}' having count(*) >= 5) as day natural join  
    (select search_string, sum(case when page_number = 0 then mysql_id else page_number * mysql_id end) as sumRank, count(*) as totalClicks from fourdays where date > ${last7Days} and date < ${nextDay} and fk_item <> 0 and search_string = '${string}') as ranking natural join
    (select search_string, count(*) as oneCount from fourdays where date > ${last7Days} and date < ${nextDay} and mysql_id = 1 and (page_number = 1 or page_number = 0) and fk_item <> 0 and search_string = '${string}') as firstOption
    `;
  },
