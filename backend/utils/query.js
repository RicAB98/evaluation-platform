const query = {
  ...
  getHotQueries(startDate) {
    return `select search_string as id, avgRank, totalLast24h, totalPrevious24h, totalLast4days, ((totalLast24h-totalPrevious24h)/totalPrevious24h  * 100) as GrowthLast24h, ((totalLast24h-average4days)/average4days * 100) as GrowthLast4d, dates, searches from  
    ...
    (select search_string, cast(avg((page_number - 1) * 10 + mysql_id) as decimal(10,2)) as avgRank from fourdays where date <> '2021-01-21' and mysql_id <> 0 group by search_string)  as ranking natural join
  ...
  getHotPages(startDate) {
    return `select concat(tp_item,', ', fk_item) as id, avgRank, totalPrevious24h, totalLast24h, totalLast4days, ((totalLast24h-totalPrevious24h)/totalPrevious24h  * 100) as GrowthLast24h, ((totalLast24h-average4days)/average4days * 100) as GrowthLast4d, dates, searches from  
