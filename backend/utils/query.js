const query = {
  ...
  getQuerySummary(string, halfHourAgo, oneHourAgo, twoHoursAgo, startDate, endDate, nextDay, last24Hours, last7Days) {
    ...
    endTime = `'${endDate.getFullYear()}-${endDate.getMonth() + 1}-${endDate.getDate()} ${endDate.getHours()}:${endDate.getMinutes()}:${endDate.getSeconds()}'`;
    ...
    last7DaysTime = `'${last7Days.getFullYear()}-${last7Days.getMonth() + 1}-${last7Days.getDate()} 00:00:00'`;
    ...
    (select search_string, sum(case when page_number = 0 then mysql_id else page_number * mysql_id end) as sumRank, count(*) as totalClicks from zzlog_search where time > ${last7DaysTime} and time < ${endTime} and fk_item <> 0 and search_string = '${string}') as ranking on defaultTable.search_string = ranking.search_string left join
    ...
    (select search_string, count(*) as oneCount from zzlog_search where time > ${last7DaysTime}  and time < ${endTime} and mysql_id = 1 and (page_number = 1 or page_number = 0) and fk_item <> 0 and search_string = '${string}') as firstOption on defaultTable.search_string = firstOption.search_string
    `;
  },
  ...
  getPageSummary(tp_item, fk_item, halfHourAgo, oneHourAgo, twoHoursAgo, startDate, endDate, nextDay, last24Hours, last7Days) {
    ...
    (select tp_item, fk_item, sum(case when page_number = 0 then mysql_id else page_number * mysql_id end) as sumRank, count(*) as totalClicks from zzlog_search where date > ${last7Days} and date < ${endTime} and fk_item <> 0 and tp_item = ${tp_item} and fk_item = ${fk_item}) as ranking left join
    ...
    (select tp_item, fk_item, count(*) as oneCount from zzlog_search where date > ${last7Days} and date < ${endTime} and mysql_id = 1 and (page_number = 1 or page_number = 0) and fk_item <> 0 and tp_item = ${tp_item} and fk_item = ${fk_item}) as firstOption on ranking.tp_item = firstOption.tp_item and ranking.fk_item = firstOption.fk_item
    `;
  },
  ...
  getHotPages(minimum, halfHourAgo, oneHourAgo, twoHoursAgo, startDate, nextDay, last24Hours, last7Days) {
    ...
    (select tp_item, fk_item, count(*) as oneCount from zzlog_search where date > ${last7Days} and date < ${nextDay} and mysql_id = 1 and (page_number = 1 or page_number = 0) and fk_item <> 0 group by tp_item, fk_item) as firstOption on day.tp_item = firstOption.tp_item and day.fk_item = firstOption.fk_item left join
